...
(function () {
  const ROUTE_MAP = {
    ...
    'pipeline':          { href: 'pipeline-flow.html',      crumb: ['홈', '추천 & 분석', '파이프라인 흐름도'] },
  ...
  const ICONS = {
    ...
    pipeline:    '<svg viewBox="0 0 24 24"><rect x="2" y="4" width="5" height="5" rx="1"/><rect x="9.5" y="4" width="5" height="5" rx="1"/><rect x="17" y="4" width="5" height="5" rx="1"/><path d="M7 6.5h2.5M14.5 6.5H17"/><rect x="5.5" y="15" width="5" height="5" rx="1"/><rect x="13.5" y="15" width="5" height="5" rx="1"/><path d="M4.5 9v3h8V9M14 9v3M14.5 12v3M8 12v3"/></svg>',
    ...
    return `
      <aside class="side">
        ...
        <div class="grp">
          ...
          ${buildItem('pipeline', '파이프라인 흐름도', null, activeRoute === 'pipeline', base)}
        </div>
    ...
    return `
      <div class="topbar">
        ...
        <div class="top-right">
          ...
          <div class="top-user">
            <div class="nm-t" id="top-adv-name">—<small>광고주</small></div>
          </div>
        </div>
      </div>
    `;
  ...
  function init() {
    ...
    // 광고주명 sidebar + topbar에 반영
    const adv = getAdv();
    const advLabel = adv ? adv.name : '—';
    const nameEl = document.getElementById('side-adv-name');
    if (nameEl) nameEl.textContent = adv ? `#${adv.id} ${adv.name}` : '—';
    const topEl = document.getElementById('top-adv-name');
    if (topEl) topEl.innerHTML = `${advLabel}<small>광고주</small>`;
  }
